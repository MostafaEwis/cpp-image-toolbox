...
#include <gl/glut.h>
using namespace std;
//intialize the pixel vector with width and height so you don't have to copy more data
//ifstream practices
//show image
//write output images
...
namespace hexTools{
	...
	int hexToInt(string hexWord){
		int res = 0;
		for(size_t i = 0; i < hexWord.size(); i++) res += hexCharToInt(hexWord[i]) * (int)pow(16.0, (double)i);
		return res;
	}
